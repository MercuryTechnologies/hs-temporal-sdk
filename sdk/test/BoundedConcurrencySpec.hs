...
import Temporal.Exception (ActivityFailure, WorkflowExecutionClosed (..))
...
workflowTests = do
  ...
  describe "waitConcurrentlyBounded" $ do
    specify "an uncaught waitAction exception aborts the drain" $ \TestEnv {..} -> do
      let wf :: W.ProvidedWorkflow (W.Workflow (ActionResult Int [Int]))
          wf =
            W.provideWorkflow defaultCodec "boundedDrainThrows" $
              provideCallStack $
                let unfold :: Int -> W.Workflow (Maybe (Int, Int))
                    unfold i = pure $ if i > 3 then Nothing else Just (i, i + 1)
                    waitA :: Int -> W.Workflow Int
                    waitA i = if i == 1 then throwM (userError "boom") else pure i
                 in waitConcurrentlyBounded @2 unfold 1 waitA
          conf = configure () wf $ do baseConf
      withWorker conf $ do
        let opts = defaultStartOptsWithTimeout taskQueue (seconds 30)
        useClient (C.execute wf.reference "boundedDrainThrows" opts)
          `shouldThrow` \case
