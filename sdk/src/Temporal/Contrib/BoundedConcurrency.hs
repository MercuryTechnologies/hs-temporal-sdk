...
import Temporal.Workflow.Internal.Inflight (Inflight)
...
waitConcurrentlyBoundedGen WaitConcurrentlyBoundedArgs {..} = do
  let maxConcurrent :: Int = fromIntegral (natVal' (proxy# @n))
      step :: RequireCallStack => Inflight b -> runningState -> ActionResult unfoldState runningState -> (Inflight b -> runningState -> Workflow (ActionResult unfoldState result)) -> Workflow (ActionResult unfoldState result)
      step inf runningState nothingK justK =
        Inflight.next inf >>= \case
          Nothing -> traverse prepareResult nothingK
          Just (Left e, _) -> throwM e
          Just (Right firstResult, inf') -> justK inf' =<< finishedWaiterAction runningState firstResult

      drain :: RequireCallStack => Inflight b -> runningState -> (runningState -> ActionResult unfoldState runningState) -> Workflow (ActionResult unfoldState result)
      drain inf runningState mkResult = step inf runningState (mkResult runningState) (\inf' runningState' -> drain inf' runningState' mkResult)

      decide :: RequireCallStack => Inflight b -> unfoldState -> runningState -> Workflow (ActionResult unfoldState result)
      decide inf unfoldState runningState = do
        historyExhausted <- continueAsNewSuggested <$> info
        case historyExhausted of
          True -> drain inf runningState (ActionResult'HistoryExhausted unfoldState)
          False -> step inf runningState (ActionResult'Done runningState) \inf' runningState' ->
            unfoldAction unfoldState >>= \case
              Nothing -> drain inf' runningState' ActionResult'Done
              Just (a, unfoldState') -> do
                inf'' <- Inflight.push inf' (waitAction a)
                decide inf'' unfoldState' =<< newWaiterAction runningState' a

  let initLoop :: Int -> unfoldState -> runningState -> Inflight b -> Workflow (runningState, unfoldState, Inflight b)
      initLoop remainingIters unfoldState runningState inf = case remainingIters <= 0 of
        True -> pure (runningState, unfoldState, inf)
        False ->
          unfoldAction unfoldState >>= \case
            Nothing -> pure (runningState, unfoldState, inf)
            Just (a, unfoldState') -> do
              let !remainingIters' = remainingIters - 1
              runningState' <- newWaiterAction runningState a
              inf' <- Inflight.push inf (waitAction a)
              initLoop remainingIters' unfoldState' runningState' inf'

  (runningState, unfoldState, inf) <- initLoop maxConcurrent initialUnfoldState initialRunningState Inflight.empty
  decide inf unfoldState runningState
